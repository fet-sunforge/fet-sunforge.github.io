...
export const courses = ref([{
  ...
  code: "beeefyp1",
  projects: [{
    title: 'A Contactless Health Monitoring System Using Thermal and Motion Sensors',
    ...
    title: 'Experimental Characterization of PCB Antenna-based PUFs for IoT Device Authentication',
  ...
  projects: [{
    ...
    title: 'Module-Level Identification Using Intrinsic RF Properties of Antennas for PUF-Based Security ',
    ...
    title: 'Optical fiber sensing for Detection of Liquid Leakage',
    ...
    title: 'A Fibre Optic-Based Intrusion Detection System with WPD and Ensemble Learning',
    desc: 'To address the growing demand for perimeter intrusion security, this project first utilised a Distributed Acoustic Sensor (DAS) to measure vibration signals generated by intrusion events (climbing and striking) and environmental events (background noise, wind, and thunderstorms) on fences. The distinctive vibration signals from all five event types were denoised using wavelet packet decomposition (WPD) and normalised using min–max scaling. Next, fixed-length samples were extracted using the zero-crossing rate method, resulting in a total of 1,468 samples. The labelled samples for each disturbance event were then fed into an ensemble learner, which employed a decision tree and a support vector classifier as the base-level (level-0) learners, and logistic regression as the meta-level (level-1) learner. The ensemble learner achieved an overall testing accuracy of 87.67% and attained at least 88.5% accuracy in suppressing false alarms caused by environmental events (background noise, wind, and thunderstorms). By reducing false alarms, the system decreases operator desensitisation. DAS promotes sustainability as it uses fibre-optic cables as sensors, which offer low scalability cost and are easy to install and maintain, contributing to SDG Goal 7. Fibre-optic sensing is also safe, as it operates at low voltage and is immune to electromagnetic interference (EMI). Finally, this system is envisioned to operate as a real-time intrusion detection solution.',
    pic: ['Ooi Ke Vin']
  }, {
  ...
  projects: [{
    ...
    title: 'Control challenges of the new extractive–reactive distillation: An alternative workaround for stoichiometric ratio control',
